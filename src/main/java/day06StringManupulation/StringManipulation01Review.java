...
public class StringManipulation01Review {
    public static void main(String[] args) {
       ...
       // 1.way : By using charAt() method    note :index of last character ==> length()-1
      char firstChar =  str.charAt(0);

     char lastChar = str.charAt(str.length()-1);  //if you use char dt in a math operation java uses ascii values
    ...
    // 2.way by using substring()
      ...
      String firstCharacter = str.substring(0,1);//gives you first character everytime

      String lastCharacter=  str.substring(str.length()-1);//beginning index gives you the character from a specific index to the last index

        System.out.println(firstCharacter+lastCharacter);


        //print just the animal names from the string " I like lion, horse, laptop and cat"
    ...
    String initialOfLastName=  name.split(" ")[1].substring(0,1);
        ...
        int numOfWord =word.split(" ").length;
