...
public class Lambda01 {
    ...
    It saves a lot of code. In case of lambda expression, we don't need to define the method again for providing
     ...
     Lambda expressions are similar to methods, but they do not need a name , they can be implemented
   ...
   //And then return  the greater value with the help of Ternary Structure
    ...
    public static void getMaxElement01(List<Integer> list){
       Integer maxNum= list.stream().distinct().reduce(Integer.MIN_VALUE,(t,u) -> t>u ? t:u);
        System.out.println(maxNum);
    }
    public static void getMaxElement02(List<Integer> list){
        Integer maxNum2=list.stream().distinct().sorted().reduce(Integer.MIN_VALUE,(t,u) -> u);//sorted() always puts the ele.s in ascending
        System.out.println(maxNum2);
    }
   // Create a method to find the min value from  the list elements (in 2.way) HOMEWORK
